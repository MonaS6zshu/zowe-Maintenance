...
gulp.task('applyCheck', 'Apply Check Maintenance', function (callback) {
  var ds = config.remoteJclPds + '(' + config.applyCheckMember + ')';
  submitJobAndDownloadOutput(ds, "job-archive/applyCheck", 0, callback);
});
...
gulp.task('upload', 'Upload to the USS', function (callback) {
